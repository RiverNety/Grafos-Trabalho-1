...
int main() {
    ...
    for (int i = 1; i <= 6; ++i) {
        ...
        int diametro = grafo.calcular_diametro_aproximado();

        std::ofstream saida(pasta_saida + "/resultado.txt");
        saida << "Grafo: " << nome_grafo << "\n";
        saida << "Diametro aproximado: " << diametro << "\n";
        saida.close();

        std::cout << "Grafo " << i << ": diametro aproximado = " << diametro << "\n";
    }
